...
class Agent:
    ...
    def train_policy_model(self):
        ...
        for i_episode in range(self.num_episodes):
            ...
            BufferState=torch.zeros(1500,4, device=device)
            Mean=torch.zeros(1500,2, device=device)
            Var = torch.zeros(1500,2, device=device)
            SMean=torch.zeros(400,2, device=device)
            SVar = torch.zeros(400,2, device=device)
            Sstate = torch.zeros(400,4,device=device)
            ...
            for t in count():
                ...
                BufferState[t]=state
                print(state)
                action , Mean[t] , Var[t] = self.select_action(self.process_state(state))
        ...
        #Le variabili ci sono, bisogna plottare e salvare i grafici

        self.save_model()
        print('Training completed')
        plt.figure(figsize=(10,5))
        plt.plot(range(len(self.time_train_mean)), self.time_train_mean, 'steelblue')
        plt.plot(range(len(Svar)), self.time_train_mean, 'steelblue')
